...
@section('content')
    ...
    <div class="mt-2">
        ...
        <x-form-section>
            ...
            <x-slot name="title">{{ __("Create a new jail") }}</x-slot>


            <x-slot name="description">
                {{ __("You can register a new jail.") }}
            </x-slot>
                ...
                <form  method="POST" action="{{ route('jail.store') }}" class="grid grid-cols-6 gap-6">
                    ...
                    <div class="col-span-6 sm:col-span-3">
                        ...
                        <x-select name="type" id="type" class="block mt-2 w-full" required>
                            ...
                            <option value="low" {{ old('type') === 'low' ? 'selected' : ''}}>
                                {{ __('Low') }}
                            </option>

                            <option value="medium" {{ old('type') === 'medium' ? 'selected' : ''}}>
                                {{ __('Medium') }}
                            </option>

                            <option value="high" {{ old('type') === 'high' ? 'selected' : ''}}>
                                {{ __('High') }}
                            </option>
                        </x-select>
                    ...
                    <div class="col-span-6 sm:col-span-3">
                        ...
                        <x-select name="ward_id" id="ward_id" class="block mt-2 w-full" required>
                            ...
                            @foreach($wards as $ward)
                                <option value="{{ $ward->id }}" {{ old('ward') === (string) $ward->id ? 'selected' : ''}}>
                                    {{ $ward->name }}
                                </option>
                            @endforeach
                        </x-select>
